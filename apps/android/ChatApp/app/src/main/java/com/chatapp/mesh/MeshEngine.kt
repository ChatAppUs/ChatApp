...
class MeshEngine(
    ...
    /** Per-source relay rate-limiter (abuse prevention; see MeshRelayLimit.kt). */
    val relayLimiter = RelayLimiter()

    /** Byte-based radio backpressure (see MeshCongestion.kt). */
    val congestion = CongestionController()

    /** Proactive route-repair state machine (see MeshRouteRepair.kt). */
    val repairSM = RepairStateMachine()

    /** EWMA-smoothed link quality per neighbour (see MeshLinkFeedback.kt). */
    val linkFeedback = LinkFeedback()

    /** Explicit multipath relay selection (see MeshMultipath.kt). */
    val multipath = MultipathSelector()

    /** Battery-aware scan and quota scaling (see MeshPower.kt). */
    val powerManager = PowerManager()

    /** Per-device-count TTL budget (see MeshScale.kt). */
    fun scaledMaxHops(): Int = MeshScale.hopsForDevices(neighbors.size)
