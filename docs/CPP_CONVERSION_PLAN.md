...
| 3 | `services/sfu/` (Go/Pion: signaling + embedded STUN/TURN + HMAC room tickets) | RTP/RTCP forwarding hot loop to C++ | 🚧 In progress — C++ TURN relay data plane shipped (`services/sfu-forwarder`, RFC 5389/5766 subset: allocate/create-permission/channel-bind/channel-data + Send/Data indications, self-contained SHA-1/HMAC, zero external deps; 18/18 raw-socket checks in `tests/sfu_turn_test.py`). Go API prefers it via `TURN_FORWARDER` env with pion/turn fallback. RTP/RTCP relay plane remains Go/Pion until saturation. |
