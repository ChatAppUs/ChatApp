...
func (a *App) handleAdminSecurityAttestations(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		writeErr(w, http.StatusInternalServerError, "failed to load security attestations")
		return
	}
	defer rows.Close()
	type item struct {
	...
	for rows.Next() {
		var x item
		if err := rows.Scan(&x.ID, &x.UserID, &x.Username, &x.Purpose, &x.Score, &x.VerifiedAt, &x.ExpiresAt); err == nil {
