...
func main() {
	...
	mux.HandleFunc("GET /api/me/flags", app.requireAuth(app.handleMyFlags))
	mux.HandleFunc("GET /api/me/experiments", app.requireAuth(app.handleMyExperiments))
	mux.HandleFunc("POST /api/telemetry/qoe", app.requireAuth(app.handleQoEIngest))
	mux.HandleFunc("POST /api/telemetry/call-quality", app.requireAuth(app.handleCallQualityIngest))
	mux.HandleFunc("GET /api/admin/flags", app.requireAdminPerm("platform.manage")(app.handleAdminListFlags))
	mux.HandleFunc("POST /api/admin/flags", app.requireAdminPerm("platform.manage")(app.handleAdminUpsertFlag))
	mux.HandleFunc("PUT /api/admin/flags/{key}", app.requireAdminPerm("platform.manage")(app.handleAdminUpdateFlag))
	mux.HandleFunc("DELETE /api/admin/flags/{key}", app.requireAdminPerm("platform.manage")(app.handleAdminDeleteFlag))
	mux.HandleFunc("GET /api/admin/experiments", app.requireAdminPerm("platform.manage")(app.handleAdminListExperiments))
	mux.HandleFunc("POST /api/admin/experiments", app.requireAdminPerm("platform.manage")(app.handleAdminCreateExperiment))
	...
	mux.HandleFunc("GET /api/miniapps", app.requireAuth(app.handleListMiniApps))
	...
	mux.HandleFunc("GET /api/auth/2fa/recovery-codes", app.requireAuth(app.handleListRecoveryCodes))
	mux.HandleFunc("POST /api/auth/2fa/recovery-codes/redeem", app.requireAuth(app.handleRedeemRecoveryCode))
	mux.HandleFunc("POST /api/auth/2fa/recovery-codes/disable", app.requireAuth(app.handleDisable2FAWithRecovery))
	mux.HandleFunc("POST /api/messages/{id}/unsend", app.requireAuth(app.handleUnsendMessage))
	mux.HandleFunc("GET /api/messages/{id}/edits", app.requireAuth(app.handleMessageEditHistory))
	mux.HandleFunc("POST /api/stories/{id}/archive", app.requireAuth(app.handleArchiveStory))
	mux.HandleFunc("GET /api/me/story-archive", app.requireAuth(app.handleListStoryArchive))
	mux.HandleFunc("GET /api/me/memories", app.requireAuth(app.handleListMemories))
	mux.HandleFunc("DELETE /api/me/memories/{id}", app.requireAuth(app.handleDeleteMemory))
	mux.HandleFunc("POST /api/reels/drafts", app.requireAuth(app.handleSaveReelDraft))
	mux.HandleFunc("GET /api/reels/drafts", app.requireAuth(app.handleListReelDrafts))
	mux.HandleFunc("DELETE /api/reels/drafts/{id}", app.requireAuth(app.handleDeleteReelDraft))
	mux.HandleFunc("POST /api/reels/captions", app.requireAuth(app.handleSaveReelCaption))
	mux.HandleFunc("GET /api/reels/{id}/caption", app.requireAuth(app.handleGetReelCaption))
	mux.HandleFunc("GET /api/creator/insights", app.requireAuth(app.handleCreatorInsights))
	mux.HandleFunc("POST /api/share", app.requireAuth(app.handleCreateShareToken))
	mux.HandleFunc("GET /api/share/{token}", app.requireAuth(app.handleGetShare))
	mux.HandleFunc("POST /api/calls/events", app.requireAuth(app.handleCallEvent))
	mux.HandleFunc("GET /api/calls/{room}/events", app.requireAuth(app.handleCallEvents))
	mux.HandleFunc("POST /api/calls/{room}/rating", app.requireAuth(app.handleRateCall))
	mux.HandleFunc("GET /api/calls/{room}/rating", app.requireAuth(app.handleGetCallRating))
	mux.HandleFunc("POST /api/calls/schedule", app.requireAuth(app.handleScheduleCall))
	mux.HandleFunc("GET /api/calls/scheduled", app.requireAuth(app.handleListScheduledCalls))
	mux.HandleFunc("DELETE /api/calls/scheduled/{id}", app.requireAuth(app.handleCancelScheduledCall))
	mux.HandleFunc("GET /api/pages/{id}/insights", app.requireAuth(app.handlePageInsights))
	mux.HandleFunc("PUT /api/groups/{id}/approval", app.requireAuth(app.handleSetGroupPostApproval))
