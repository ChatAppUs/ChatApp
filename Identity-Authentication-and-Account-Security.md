...
| Email/phone/password credential-change verification with OTP, KYC face match, and five-second liveness | Implemented with runtime validation pending | `handlers_credential_security.go`, `handlers_security_attestation.go`, `028_credential_change_challenges.sql`, `029_security_attestations.sql`, and `apps/web/src/app/settings/page.tsx` provide server-owned OTP challenges, fresh-selfie ML attestation, password/contact updates, session revocation, the 48-hour withdrawal freeze, and a functional web password-change panel. | Run database-backed flows with configured ML, SMTP, and SMS services; extend the same client flow to native clients. |
