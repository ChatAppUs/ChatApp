...
| Account deletion with email OTP, phone OTP, liveness, asset confirmation, 30-day cancellation, and permanent deletion | Partially implemented | `services/api/handlers_identity_lifecycle.go` and migration `026_identity_security_lifecycle.sql` implement password proof, asset confirmation, KYC gate, 30-day scheduling, cancellation-on-login, session revocation, status, and permanent cleanup. | Add server-owned email OTP, phone OTP, and liveness/face-match verification before marking complete. |
