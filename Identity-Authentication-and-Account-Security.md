...
| Account deletion with email OTP, phone OTP, liveness, asset confirmation, 30-day cancellation, and permanent deletion | Partially implemented; server-owned challenges added | `handlers_identity_lifecycle.go`, `handlers_deletion_verification.go`, `026_identity_security_lifecycle.sql`, and `027_deletion_verification_challenges.sql` implement password proof, asset confirmation, KYC gate, hashed/expiring email and phone OTPs, a tracked liveness challenge, 30-day scheduling, cancellation-on-login, session revocation, status, and permanent cleanup. | Replace the liveness challenge response with configured provider-backed face-match/liveness attestation before marking complete. |
