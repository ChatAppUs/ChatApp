...
| Email/phone/password credential-change verification with OTP, KYC face match, and five-second liveness | Implemented with runtime validation pending | `handlers_credential_security.go`, `handlers_security_attestation.go`, `028_credential_change_challenges.sql`, `029_security_attestations.sql`, `apps/web/src/app/settings/page.tsx`, `apps/android/.../ApiClient.kt`, and `apps/ios/.../FeatureClient.swift` provide server-owned OTP challenges, fresh-selfie ML attestation, password/contact updates, session revocation, the 48-hour withdrawal freeze, a functional web panel, and Android/iOS API bindings. | Run database-backed flows with configured ML, SMTP, and SMS services; complete native UI wiring and desktop/admin/extension client flows. |
...
| Cross-platform identical feature parity and production operations | Partially implemented | The parity scanner passes route coverage; Android and iOS expose identity-security API bindings, the Tauri desktop shell inherits the web security panel, and the extension options page now exposes OTP, fresh-selfie attestation, password-change, and deletion-status controls through `apps/extension/security.js`. Full native UI and runtime parity is not proven; admin remains scoped to administrative operations. | Add complete native UI, define required admin audit views, and run cross-platform runtime, load, security, observability, backup, and restore validation. |
