...
| Email/phone/password credential-change verification with OTP, KYC face match, and five-second liveness | Implemented with runtime validation pending | `handlers_credential_security.go`, `handlers_security_attestation.go`, `028_credential_change_challenges.sql`, and `029_security_attestations.sql` add server-owned current/new email or phone OTP challenges, fresh-selfie ML face-match/liveness attestation, password/contact updates, session revocation, and the 48-hour withdrawal freeze. | Run database-backed flows with configured ML, SMTP, and SMS services. |
