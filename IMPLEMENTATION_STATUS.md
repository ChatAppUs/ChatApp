...
|---|---|---|---|
| Unified authentication and account security | Implemented with runtime validation pending | `handlers_credential_security.go`, `handlers_security_attestation.go`, `028_credential_change_challenges.sql`, `029_security_attestations.sql`, `apps/web/src/app/settings/page.tsx`, Android `ApiClient.kt`, iOS `FeatureClient.swift`, and the auth service. Email/phone/password changes now require server-owned OTPs plus fresh-selfie ML attestation, revoke sessions, apply the 48-hour withdrawal freeze, expose a web panel, and have Android/iOS API bindings. | Run Go/Rust/native tests and database-backed flows with ML, SMTP, and SMS services; complete native UI and desktop/admin/extension flows. |
...
| Cross-platform identity-security parity | Partially implemented | Android and iOS expose API bindings, the Tauri desktop shell inherits the web security panel, and the extension options page now exposes OTP, fresh-selfie attestation, password-change, and deletion-status controls through `apps/extension/security.js`; admin remains scoped to administrative operations. Full native UI and runtime parity is not proven. | Add complete native UI, define required admin audit views, and run cross-platform runtime validation. |
