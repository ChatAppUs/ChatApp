...
| Cross-platform identity-security parity | Partially implemented | Web, Android, and iOS expose identity-security controls; the Tauri desktop shell inherits the web security panel; the extension options page exposes OTP, fresh-selfie attestation, password-change, and deletion-status controls; and admin has a read-only `/api/admin/security/attestations` audit endpoint plus dashboard view. Full runtime parity is not proven. | Run cross-platform runtime validation and complete any remaining operational audit requirements. |
