...
| Cross-platform identity-security parity | Partially implemented | Android and iOS expose API bindings, the Tauri desktop shell inherits the web security panel, and the extension exposes shared server-owned bindings in `apps/extension/security.js`; admin remains scoped to administrative operations. Full native/extension UI and runtime parity is not proven. | Add complete native/extension UI, define required admin audit views, and run cross-platform runtime validation. |
