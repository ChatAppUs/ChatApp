...
|---|---|---|---|
| Unified authentication and account security | Implemented with runtime validation pending | `handlers_credential_security.go`, `handlers_security_attestation.go`, `028_credential_change_challenges.sql`, `029_security_attestations.sql`, `apps/web/src/app/settings/page.tsx`, and the auth service. Email/phone/password changes now require server-owned OTPs plus fresh-selfie ML attestation, revoke sessions, apply the 48-hour withdrawal freeze, and expose a functional web password-change panel. | Run Go and Rust unit/integration tests in an environment with Go and Cargo installed; run database-backed flows with ML, SMTP, and SMS services; extend the same client flow to native clients. |
...
| Cross-platform identity-security parity | Not complete | Android and iOS expose sessions/2FA, while the new credential-change, biometric-attestation, and account-deletion verification controls are currently web-only; desktop, admin, and extension do not expose the complete identity-security flow. | Implement equivalent native, desktop, admin, and extension flows before marking parity complete. |
