...
| Cross-platform identity-security parity | Partially implemented | Web, Android, and iOS expose identity-security controls; the Tauri desktop shell inherits the web security panel; and the extension options page exposes OTP, fresh-selfie attestation, password-change, and deletion-status controls. Admin remains scoped to administrative operations. Full runtime parity is not proven. | Run cross-platform runtime validation and define any required admin audit views. |
