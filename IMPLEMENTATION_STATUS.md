...
|---|---|---|---|
| Unified authentication and account security | Implemented with runtime validation pending | `services/api/handlers_auth.go`, `handlers_security.go`, `auth.go`, `handlers_webauthn.go`, `handlers_qrlogin.go`, `handlers_oauth.go`, `services/authn/` | Run Go and Rust unit/integration tests in an environment with Go and Cargo installed; run database-backed auth flows. |
...
| KYC and withdrawal/security gates | Implemented with runtime validation pending | `handlers_features.go`, `handlers_wallet.go`, `handlers_crypto.go`, `handlers_p2p.go`, `handlers_staking.go`, `handlers_cards.go`, `handlers_admin.go` and related migrations. | Validate ML/admin review and all financial authorization paths with seeded data. |
