...
Verified directly against the executable source, not from any status document. Mesh congestion control (previously listed as still absent) is implemented in `services/mesh/congestion.go` — a token-bucket byte budget enforced in the node send path, with per-tick flush so blocked packets recover and admission/block counters in queue status. ACKs are now AEAD-sealed under the per-peer session key so delivery credit cannot be forged. Migrations `040_mesh_device_ownership.sql` and `041_mesh_relay_holder.sql` plus the rewritten `/api/mesh/*` handlers bind devices to persistent per-client identity keys; Android, iOS and the web mesh page generate and persist a local mesh identity key and present it on every mesh call. The replay filter is bounded (LRU peer eviction, empty-source rejection, one-hour idle expiry). `services/sfu-forwarder/main.cpp` had a use-before-definition of `constantTimeEqual` that broke the strict C++17 `-Werror` build; it is fixed and all five C++ data planes compile.

Environment validation executed in this checkout: Go `build`/`vet`/`test` pass for `services/api`, `services/mesh` (including `go test -race` and `gofmt`) and `services/sfu`; all five C++17 services compile under `-Wall -Wextra -Werror`; the web production build generates **54 routes** and the admin build passes; parity passes with **155 files / 547 registered routes**; the feature registry passes with 26 features across 7 required clients; all 41 migrations apply cleanly (214 tables); Python ML, extension syntax and `git diff --check` pass; and **21/21 Python E2E suites pass with zero failures** against a live API on PostgreSQL 15.19 with the Go SFU and the C++ TURN forwarder running. Route repair, multipath selection, group sender-key rotation, group acknowledgements, and network-wide revocation distribution are now implemented (see the 2026-09-15 gap-closure pass). Tor/onion transport, physical radio validation, native release builds, configured providers, and production load/backup/DR certification remain open.
...
| Offline mesh and native shared-core architecture | Implemented (backend store-and-forward + Go engine + native Bluetooth / Wi-Fi Direct transports) | `infra/db/031_mesh.sql`, `services/api/handlers_mesh.go`, and `services/api/main.go` implement device registration, encrypted store-and-forward packet enqueue/dedup, poll-based delivery, multi-hop relay with TTL/hop accounting, relay consent policy, and mesh status — all wired as `/api/mesh/*` routes. Device registration is open to anonymous clients: the account link is stored only when the token subject is a real account UUID, so guest sessions (`guest_<id>` subjects) register as pure relay/member nodes instead of failing the `uuid` cast. The Go engine in `services/mesh/` provides authenticated encryption (NaCl secretbox), local Wi-Fi/hotspot UDP transport with presence-beacon discovery, multi-hop routing with TTL and duplicate suppression, a delay-tolerant store-and-forward queue, and 1:1/group/voice/call-signaling payloads; `native_transport.go` adds the **Bluetooth (RFCOMM) and Wi-Fi Direct stream bridge transports** plus `AutoTransport`, which implements the Anonymous.md §5.3 fallback order (local Wi-Fi → Wi-Fi Direct → Bluetooth → store-and-forward) and keeps packets queued when no radio is reachable. The native clients implement the radios themselves: Android `apps/android/app/src/main/java/com/chatapp/mesh/MeshTransport.kt` (BluetoothServerSocket/BluetoothSocket RFCOMM + WifiP2pManager group socket, with `MeshEngine.kt` mirroring the Go engine's routing and queue) and iOS `apps/ios/ChatApp/Sources/Services/MeshTransport.swift` (CoreBluetooth GATT peripheral+central) with `MeshEngine.swift`. **2026-09-15 reliability pass:** `reliability.go` adds reliable delivery — a new `ack` packet kind plus `xfer`/`ack_for` envelope fields, 5 bounded retries with exponential backoff (capped at 60 s), exactly-once application delivery on the receiver while every retried copy is acknowledged, and user-visible delivery states `queued` → `relaying` → `acked` with terminal `expired` and `dead_letter`. `priority.go` and `pfifo.go` replace the count-bounded single-class forwarding queue with traffic classes drained control → text → voice → media (FIFO within a class), bounded by packet count **and** bytes, evicting lowest-priority-first and never displacing control traffic. `node_reliable.go` exposes `SendReliable` and the delivery/queue status used by `/api/mesh/*`. **2026-09-15 native session-key pass:** the Android/iOS native engines now adopt the Go engine's hardened identity path — `MeshIdentity.kt` (Android) and `MeshIdentity.swift` (iOS) implement Ed25519-signed beacons advertising an X25519 key-agreement key, per-peer AES-256 session keys via ECDH + HKDF-SHA256 (`chatapp-mesh-session-v2`), trust-on-first-use key pinning, local key revocation and per-source sliding-window replay protection, with pre-shared-key fallback for peers that have not advertised a KEM key. The Android logic is unit-tested on the JVM (20/20 checks). **2026-09-15 gap-closure pass:** congestion control (`congestion.go`), proactive route repair (`routerepair.go`), explicit multipath selection (`multipath.go`), per-group sender-key rotation (`groupkey.go`), per-member group acknowledgements (`groupack.go`), and network-wide revocation distribution (`revocdist.go`) are now implemented in `services/mesh` with executable tests. | Radio paths require on-device validation: CI has no Bluetooth/Wi-Fi Direct hardware, so the bridge, selection order and Node routing are covered by `services/mesh/native_transport_test.go` on real loopback sockets, while the radio handshakes themselves need real devices. Congestion control, route repair, multipath selection, group sender-key rotation, group acknowledgements, and network-wide revocation distribution are implemented. The 3/10/50/100/500-device hardware experiments and physical radio validation remain outstanding. |
...
| Go tests for `services/mesh` | CI-enforced: `go build`, `go vet`, and `go test` run for the native offline mesh transport engine (crypto, packet, transport, routing, store-and-forward, node, fragmentation, reliable delivery, priority queue, messages). 63 tests; green under `go test -race` and across 20 consecutive runs (the suite had been failing intermittently, which is how the alternate-path-retry defect below surfaced). |
...
Two further source-level mesh gaps were closed in this pass. Both are covered by executable tests in `services/mesh`.

**Versioned envelope.** `Packet` now carries a `Version` field and this build stamps `EnvelopeVersion` (1) on every packet it sends. `UnmarshalPacket` rejects an envelope newer than this build supports, so a future incompatible format fails closed instead of being misparsed. Version 0 is accepted for packets from pre-versioning peers, so a rolling upgrade does not partition the mesh. `Packet.Validate` additionally enforces route, identifier and fragment-metadata coherence before routing. Covered by `TestEnvelopeVersionGate` and `TestPacketValidateRejectsMalformed`.

**MTU fragmentation and reassembly.** Radio links carry far smaller datagrams than the logical payloads the mesh transports. `fragment.go` splits a payload larger than `DefaultMaxPayload` (512 bytes) into fixed-size fragments, each encrypted with its own AEAD nonce and carried as an ordinary packet, so routing, TTL, hop counting, duplicate suppression, relay quotas and store-and-forward all apply unchanged. The receiver reassembles under bounded memory (256 groups / 8 MiB) and time (5-minute expiry), ignores duplicate fragments, drops contradictory or out-of-range metadata, and verifies a SHA-256 digest committed by the sender before the payload reaches the application. A payload that already fits in one datagram keeps the original single-packet wire shape, so small messages stay byte-for-byte interoperable with pre-fragmentation peers. `Node.Send`/`Node.SendGroup` fragment automatically above the ceiling and `Node.OpenAssemblies` reports in-flight reassembly.

Covered by ten tests in `services/mesh/fragment_test.go`, including an out-of-order multi-fragment round trip through the production node API across a three-node chain that asserts exactly-once application delivery, plus duplicate-fragment handling, digest-mismatch rejection, contradictory-metadata rejection, bounded-memory refusal and incomplete-group expiry.

**Latent data race fixed.** Race-enabled testing (which CI does not run) exposed a pre-existing race in `UDPTransport`: the receive goroutine read the inbound callback unlocked while `SetInbound` wrote it under the mutex. The reader now loads the callback under the lock and invokes it outside it, so the race is gone while a handler that replies still cannot deadlock against the lock. The full `services/mesh` suite passes under `go test -race`.

**Still open (narrowed 2026-09-15).** The 3/10/50/100/500-device hardware experiments remain outstanding. Fragmentation makes a large payload transportable across a small-MTU link; it does not create bandwidth and is not evidence of 500-device capacity. Physical Bluetooth/Wi-Fi Direct validation still requires real devices. **ACK/retry with exponential backoff, user-visible delivery states (`queued`/`relaying`/`acked`/`expired`/`dead_letter`), priority traffic classes, native Android/iOS adoption of the session-key envelope (signed beacons, per-peer session keys, local revocation, replay protection), congestion control, route repair, multipath selection, group sender-key rotation, group acknowledgements, and network-wide revocation distribution are now implemented** — see the 2026-09-15 reliability and gap-closure passes below.
...
source. The ACK/retry/delivery-state/priority-class cluster that the mesh documents list as
outstanding was confirmed genuinely absent and is now implemented in first-party Go, standard
library only:

- `services/mesh/reliability.go` — sender-side transfer state machine: acknowledgements,
  5 bounded retries with exponential backoff capped at 60 s, and the user-visible states
  `queued` → `relaying` → `acked` with terminal `expired` and `dead_letter`.
- `services/mesh/priority.go` + `pfifo.go` — priority traffic classes (control → text → voice
  → media, FIFO within a class) in a forwarding buffer bounded by packet count *and* bytes,
  evicting lowest-priority-first and never displacing control traffic.
- `services/mesh/node_reliable.go` — `SendReliable`, the retry loop, delivery-state and queue
  accessors used by the `/api/mesh/*` status surface.
- `services/mesh/reliability_test.go` — 18 tests, including an end-to-end acknowledgement round
  trip between two real nodes and an alternate-path retry assertion.
- This pass also found and fixed a defect that clobbered the recorded hop before a retry could
  consult it, silently disabling alternate-path retry; the suite had been failing
  intermittently, which is how it surfaced.

**Still absent, and not claimed:** the 3/10/50/100/500-device hardware experiments and physical radio validation. Congestion control, route repair, multipath selection, group sender-key rotation, group acknowledgements, and network-wide revocation distribution are now implemented (see the 2026-09-15 gap-closure pass). Radio handshakes still require real devices.
...
planes running; route parity 153 files / 547 routes; feature registry 26 features / 7 clients.

**Validation this pass:** fresh PostgreSQL 15.19 with all 39 migrations (214 tables); Go SFU, C++ TURN forwarder and API live; `services/mesh` build, `vet`, `gofmt` and full suite under `go test -race`; `services/api` and `services/sfu` Go suites; route parity (153 files / 547 registered routes); feature-registry validation; and the complete 21-suite Python E2E harness.
