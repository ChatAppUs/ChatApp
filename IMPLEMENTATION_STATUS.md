...
|---|---|---|---|
| Unified authentication and account security | Implemented with runtime validation pending | `handlers_credential_security.go`, `028_credential_change_challenges.sql`, `handlers_auth.go`, `handlers_security.go`, `auth.go`, `handlers_webauthn.go`, `handlers_qrlogin.go`, `handlers_oauth.go`, and `services/authn/`. Email/phone/password changes now require server-owned verification, revoke sessions, and apply the 48-hour withdrawal freeze. | Run Go and Rust unit/integration tests in an environment with Go and Cargo installed; run database-backed auth flows. |
